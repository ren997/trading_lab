...
- `analyze_trades.py`：读取交易日志并输出基础统计，包括总 R、平均 R、胜率、按 setup / market 拆分表现等。按当前约定，日志日期为 2026-07-02（含）及以前的交易只保留存档，不纳入统计。
- `trade_log_web.py`：本地填写页面。通过浏览器填表，并自动追加到 `my_trade_log.csv`。
- `错误交易记录.md`：面向日常复盘的错误链、拦截规则和当前账户纪律；出现冲动开仓念头时优先阅读。
...
- `风险金额状态` 会自动标为 `已记录` 或 `待补`；只有计划风险金额已记录、且离场后填写净盈亏的交易，才能进入自动 R 倍数统计。
